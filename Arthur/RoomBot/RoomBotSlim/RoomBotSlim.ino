...
pin 12 DataIn (purple)
 pin 11  CLK (white)
 pin 10 load (middle one)

LedControl lc=LedControl(27,23,25,8);
...
void loop() {
  ...
  for (int thisLed = 0; thisLed < ledCount; thisLed++) {
digitalWrite(barpins[thisLed], HIGH);delay(1000);
  }
  /*
...
void IRDetected() {
  ...
  GotNew = true;  codeType = My_Decoder.decode_type;  if (codeType == UNKNOWN) {
    ...
    if (My_Decoder.decode_type == MY_PROTOCOL)  {
      ...
      switch (My_Decoder.value) {
        ...
        case PRESET_PREV:	shortWinC(); break;
        case PRESET_NEXT: shortWinO(); break;
        //case SUBTITLE:
		   case BUTTON_5:	shortBobD(); break;
        case BUTTON_1: shortBobU(); break;
  ...
  if (My_Decoder.decode_type == MY_PROTOCOL2)  {
      Serial.println("Code is for TSMARTPad");
      switch (My_Decoder.value) {
        case DOWN_ARROW2:        shortBobD(); break;
        case UP_ARROW2: shortBobU();        break;
        case LEFT_ARROW2: shortWinC();        break;
        case RIGHT_ARROW2: shortWinO(); break;
      }
    }
  }
=======
