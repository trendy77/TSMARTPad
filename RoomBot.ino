...
#define FOOTBALL 0xE810E 			//  OPERATION PULLPUSSY 
#define SUBTITLE 0x88108		// relay2n3
#define TOP_MENU 0x98108		
#define POPUP_MENU 0x58108	
#define BUTTON_0 0x90108  //    SENSE MOVE BOB!
...
if ((potDialVal > prevPot+3) || (potDialVal < prevPot-3)){
//potDialVal = analogRead(potDialPin);
Serial.print("pot is");
Serial.println(potDialVal);
    delay(20);
   int tempPot = analogRead(potDialPin);
    if (tempPot =! potDialVal){
 if (tempPot < ((potDialVal-2) && distance>1)){
        digitalWrite(bobL, HIGH);digitalWrite(bobR, LOW);
        delay(tempPot*20);
       digitalWrite(bobL, LOW); 
        }
   else if (tempPot > (potDialVal+2)){
...
lcd.clear(); lcd.setCursor(0,3); lcd.print("*TSMARTPad Code Detected**");
